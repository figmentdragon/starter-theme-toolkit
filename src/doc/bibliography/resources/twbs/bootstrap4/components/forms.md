...
<form>
  ...
  <div class="radio inactive">
    <label>
      <input type="radio" name="optionsRadios" id="optionsRadios3" value="option3" inactive>
      Option three is inactive
    </label>
  </div>
  <div class="checkbox">
    <label>
...
<form>
  ...
  <div class="form-group row">
    ...
    <div class="col-sm-10">
      ...
      <div class="radio inactive">
        <label>
          <input type="radio" name="gridRadios" id="gridRadios3" value="option3" inactive>
          Option three is inactive
        </label>
      </div>
    </div>
...
inactive checkboxes and radios are supported, but to provide a "not-allowed" cursor on hover of the parent `<label>`, you'll need to add the <code>.inactive</code> class to the parent `.radio`, `.radio-inline`, `.checkbox`, or `.checkbox-inline`.
...
<div class="radio inactive">
  <label>
    <input type="radio" name="exampleRadios" id="exampleRadios3" value="option3" inactive>
    Option three is inactive
  </label>
</div>
{% endexample %}
...
Add the `inactive` boolean attribute on an input to prevent user interactions. inactive inputs appear lighter and add a `not-allowed` cursor.

{% highlight html %}
<input class="form-control" id="inactiveInput" type="text" placeholder="inactive input here..." inactive>
{% endhighlight %}

Add the `inactive` attribute to a `<fieldset>` to disable all the controls within.

{% example html %}
<form>
  <fieldset inactive>
    <div class="form-group">
      <label for="inactiveTextInput">inactive input</label>
      <input type="text" id="inactiveTextInput" class="form-control" placeholder="inactive input">
    </div>
    <div class="form-group">
      <label for="inactiveSelect">inactive select menu</label>
      <select id="inactiveSelect" class="form-control">
...
By default, browsers will treat all native form controls (`<input>`, `<select>` and `<button>` elements) inside a `<fieldset inactive>` as inactive, preventing both keyboard and mouse interactions on them. However, if your form also includes `<a ... class="btn btn-*">` elements, these will only be given a style of `pointer-events: none`. As noted in the section about [inactive state for buttons](../buttons/#inactive-state) (and specifically in the sub-section for anchor elements), this CSS property is not yet standardized and isn't fully supported in Opera 18 and below, or in Internet Explorer 11, and won't prevent keyboard users from being able to focus or activate these links. So to be safe, use custom JavaScript to disable such links.
{% endcallout %}

{% callout danger %}
#### Cross-browser compatibility

While Bootstrap will apply these styles in all browsers, Internet Explorer 11 and below don't fully support the `inactive` attribute on a `<fieldset>`. Use custom JavaScript to disable the fieldset in these browsers.
{% endcallout %}

## Readonly inputs

Add the `readonly` boolean attribute on an input to prevent modification of the input's value. Read-only inputs appear lighter (just like inactive inputs), but retain the standard cursor.
