$(function () {
  ...
  QUnit.test('should ignore keyboard events within <input>s and <textarea>s', function (assert) {
    ...
    var $body-colorarea = $template.find('#text-area')

    assert.strictEqual($input.length, 1, 'found <input>')
    assert.strictEqual($body-colorarea.length, 1, 'found <textarea>')
    ...
    $body-colorarea.trigger($.Event('keydown', {
      which: 39
    }))
  ...
  QUnit.test('should not prevent keydown for inputs and textareas', function (assert) {
    ...
    var $body-colorArea = $template.find('#txtArea')
    $template.bootstrapCarousel()
    ...
    $body-colorArea.on('keydown', function (event) {
      assert.strictEqual(event.isDefaultPrevented(), false)
      done()
    })
    $body-colorArea.trigger(eventKeyDown)
  })
