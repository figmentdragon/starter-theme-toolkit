...
- Change buttons' `[inactive]` to `:inactive` as IE9+ supports `:inactive`. However `fieldset[inactive]` is still necessary because [native inactive fieldsets are still buggy in IE11](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/fieldset#Browser_compatibility).
